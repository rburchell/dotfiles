...
;; Set the list of agenda directories to use. More directories can be added.
...
(setq org-link-abbrev-alist
      '(("QTBUG"  . "https://bugreports.qt.io/browse/QTBUG-")))
...
;; TODO: look into this (setq ido-everywhere t)
...
;;;;;;;;;
;; block agenda stuff (http://doc.norang.ca/org-mode.html)
;;;;;;;;;
         ...
         ((tags "DEADLINE<=\"<now>\"|SCHEDULED<=\"<now>\"/-DONE-CANCELLED-MEETING"
          ...
          (tags-todo "-CANCELLED/!"
                     ((org-agenda-overriding-header "Stuck Projects")
                      ...
                      (org-agenda-sorting-strategy
                       '(category-keep))))
          (tags-todo "-COMMENT-HOLD-CANCELLED/!"
                     ((org-agenda-overriding-header "Projects")
                      (org-agenda-skip-function 'bh/skip-non-projects)
                      (org-tags-match-list-sublevels 'indented)
                      (org-agenda-sorting-strategy
                       '(category-keep))))
          (tags-todo "-COMMENT-CANCELLED/!NEXT"
                     ((org-agenda-overriding-header (concat "Project Next Tasks"
                      ...
                      (org-agenda-sorting-strategy
                       '(todo-state-down effort-up category-keep))))
          (tags-todo "-COMMENT-REFILE-CANCELLED-WAITING-HOLD/!"
...
(defun myevil-quit ()
  "Kill what I'm looking at now."
  (interactive)

  ;; If there's only one window left, kill the buffer.
  (if (one-window-p)
      (kill-buffer)

    ;; Otherwise, if the current buffer is on multiple windows, kill the single window.
    (if (> (length (get-buffer-window-list)) 1)
        (delete-window)

      ;; If it's only on this one window, kill the window and the buffer.
      (kill-buffer-and-window))))

;; :q should kill the current buffer rather than quitting emacs entirely
(evil-ex-define-cmd "q" 'myevil-quit)
;; Need to type out :quit to close emacs
(evil-ex-define-cmd "quit" 'evil-quit)

(defun myevil-tabnew ()
  "Create a new tab.  Used to implement :tabnew."
  (interactive)
  (find-file "."))

(evil-ex-define-cmd "tabnew" 'myevil-tabnew)

;; Remove ibuffer from state modes, so it will work with evil bindings
(evil-set-initial-state 'ibuffer-mode 'normal)

;; Enable evil mode in the minibuffer (part of evil-collection)
(setq evil-collection-setup-minibuffer t)


;; Force evil mode for git commit too.
(add-hook 'git-commit-mode-hook 'evil-insert-state)

;; Let esc quit pretty much anything...
...
(defun backspace-whitespace-to-tab-stop (indent-offset)
  "Delete whitespace backwards by INDENT-OFFSET, or otherwise delete one character if not at whitespace."
  (interactive)
  (if (or indent-tabs-mode
    ...
    (let ((movement (% (current-column) indent-offset))
          (p (point)))
      (when (= movement 0) (setq movement indent-offset))
...
(defun rb/c-mode-hook ()
  ...
  ;; Don't add extra indent for comments
